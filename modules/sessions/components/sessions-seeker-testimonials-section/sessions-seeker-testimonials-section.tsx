...
import { sessionsImpactStats } from "@/modules/sessions/content/sessions-impact-stats";
import { testimonialSlides } from "@/modules/sessions/content/sessions-testimonials";
...
const SEEKER_TESTIMONIALS_HEADING_ID = "sessions-seeker-testimonials-heading";

/** Same card type scale as `SeekerExperiencesSection` (“Hear what our global community has to say”). */
...
export function SessionsSeekerTestimonialsSection() {
  const theme = useTheme();

  return (
    <Box
      component="section"
      aria-labelledby={SEEKER_TESTIMONIALS_HEADING_ID}
      sx={{
        position: "relative",
        overflow: "visible",
        bgcolor: "transparent",
    ...
    >
      ...
      <Container maxWidth={pageContainerMaxWidth} sx={{ position: "relative", zIndex: 3, ...pageSectionGutterSx, py: sessionsSectionPyLg }}>
        <Typography
          id={SEEKER_TESTIMONIALS_HEADING_ID}
          component="h2"
          sx={{
        ...
        {/* Same card layout as home `SeekerExperiencesSection` testimonial grid */}
        <Box
          sx={{
            mt: { xs: 3, md: 3.5 },
            display: "grid",
        ...
        >
          {testimonialSlides.map((item) => {
            ...
            >
              ...
              {/* Sessions-only: circular avatar overlapping bottom-right (home cards omit this). */}
              <Box
                sx={{
                  position: "absolute",
                  bottom: {
